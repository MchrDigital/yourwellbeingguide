...
var initialSrc = "https://got.purewellnessdaily.site/"+campaignID+"?format=json";
...
setTimeout(function(){
	if (!urlParams.get('rtkcid')) {
		...
		xhr.onreadystatechange = function() {
			if (xhr.readyState == 4 && xhr.status == 200) {
				...
				document.querySelectorAll('a').forEach(function(el) {
					if (el.href.indexOf("got.purewellnessdaily.site/preclick")>-1) {
				...
				xhrr.open("GET", "https://got.purewellnessdaily.site/preview?clickid="+rawData.clickid)
				xhrr.send();
			}
		}
		...
		xhrTrack.open("GET", "https://got.purewellnessdaily.site/preview?clickid="+rtkClickID )
		xhrTrack.send();
		document.querySelectorAll('a').forEach(function(el) {
			if (el.href.indexOf("got.purewellnessdaily.site/preclick")>-1) {
				if (el.href.indexOf('?')>-1) {
